...
import UIKit
import QuickLook
class ChatViewModels: BaseViewModel {
...
//                    APIService.shared.pushNotificationMessage(fcmToken: self.user2?.fcmToken, uid: self.uid, title: self.user!.username, body: bodyNotification, badge: (self.user2?.totalBadge ?? 0) + 1)
            ...
            let roomData = ConverationModel(users: [uid, user2.id!], unreadArray: [0, 0])
...
//                    APIService.shared.pushNotificationMessage(fcmToken: self.user2?.fcmToken, uid: self.uid, title: self.user!.username, body: bodyNotification, badge: (self.user2?.totalBadge ?? 0) + 1)
    ...
    func createNewRoom(newRoom: ConverationModel, content: MessageModel, completion: @escaping(DocumentReference?, DocumentReference?, Error?)->Void) {
